...
namespace Hulk
{
    public class HediffComp_Rage : HediffComp
    {
        ...
        private bool hulkFlagged = false;
        ...
        public override void CompPostTick(ref float severityAdjustment)
        {
            ...
            if (Find.TickManager.TicksGame % 60 == 0)
            {
                ...
                if (RageRemaining < 0 || !Pawn.Spawned || (Pawn.GetComp<CompWerewolf>() is CompWerewolf ww && !ww.IsTransformed))
                {
                    ...
                    if (Pawn?.mindState?.mentalStateHandler?.CurState is MentalState_HulkFury fury)
                    {
                        fury.RecoverFromState();
                    }
                    if (Pawn?.GetComp<CompHulk>() is CompHulk compHulk)
                    {

                        if (compHulk.CurrentHulkForm != null) compHulk.TransformBack();
        ...
        public virtual float BaseRageDuration()
        {
            if (!baseRageDuration.HasValue)
            {
                ...
                if (Pawn?.GetComp<CompHulk>() is CompHulk compHulk && compHulk.CurrentHulkForm != null)
                {
                    math = compHulk.CurrentHulkForm.def.rageFactorPerLevel * compHulk.CurrentHulkForm.level;
                    math *= this.Props.baseRageSeconds;
                    math = Mathf.Clamp(math, 0f, compWerewolf.CurrentHulkForm.def.rageFactorPerLevelMax);
                }
